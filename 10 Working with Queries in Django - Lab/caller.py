...
def filter_authors_by_birth_year(first_year: int, last_year: int):
    authors = Author.objects.filter(birth_date__year__in=range(first_year, last_year+1)).order_by('-birth_date')
